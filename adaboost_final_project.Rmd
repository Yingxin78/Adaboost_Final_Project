...
Drop levels of class, there are only two class avaliable 
...
Train adaboost with k-fold, boosting.cv includes k-fold so vfold_cv is not necessary
...
When mfinal is 60, the error is lowest

```{r final model, warning= FALSE, message = FALSE}
# classification with adaboost
adaboost <- boosting(SCHRTSCHL~., data=charter_training_balance, boos=TRUE, mfinal=60, coeflearn = 'Breiman', control = rpart.control(cp=-1))
```
